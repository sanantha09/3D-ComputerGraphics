...
void CubeNode::Render()
{
	...
	constantBuffer.DirectionalLightColour = Vector4(Colors::Linen);
...
void CubeNode::BuildGeometryBuffers()
{
	...
	vertexBufferDescriptor.ByteWidth = sizeof(Vertex) * ARRAYSIZE(vertices1);
	...
	vertexInitialisationData.pSysMem = &vertices1;
	...
	indexInitialisationData.pSysMem = data(indices);
...
void CubeNode::BuildVertexNormals()
{
	...
	std::vector<int>contributingCounts(ARRAYSIZE(vertices1), 0);
	...
	for (size_t i = 0; i < (ARRAYSIZE(indices)); i += 3)
	{
		...
		Vector3 vertex0 = vertices1[index0].Position;
		Vector3 vertex1 = vertices1[index1].Position;
		Vector3 vertex2 = vertices1[index2].Position;
		...
		vertices1[index0].Normal += polygonNormal;
		vertices1[index1].Normal += polygonNormal;
		vertices1[index2].Normal += polygonNormal;
	...
	for (size_t i = 0; i < ARRAYSIZE(vertices1); i += 1)
	{
		if (contributingCounts[i] > 0)
		{
			vertices1[i].Normal /= (contributingCounts[i]);
			vertices1[i].Normal.Normalize();
		}
